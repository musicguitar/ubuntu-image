...
class ModelAssertionBuilder(State):
    ...
    def prepare_filesystems(self):
        ...
        self._next.append(self.pre_populate_filesystems_hooks)

    def pre_populate_filesystems_hooks(self):
        # Separate populate step for pre populate filesystems hook.
        bootfs = None
        for name, volume in self.gadget.volumes.items():
            for partnum, part in enumerate(volume.structures):
                if part.role is StructureRole.system_boot:
		...
		'UBUNTU_IMAGE_HOOK_BOOTFS': bootfs,
                'UBUNTU_IMAGE_HOOK_GADGET': os.path.join(self.unpackdir, 'gadget')}
        self.hook_manager.fire('pre-populate-filesystems', env)
